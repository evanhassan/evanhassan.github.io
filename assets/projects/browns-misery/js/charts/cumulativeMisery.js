...
export function render(sel,data){
 ...
 [{yr:1999,txt:'expansion return'},{yr:2017,txt:'0–16 low point'},{yr:2020,txt:'brief playoff reset'}].forEach(a=>{g.append('line').attr('x1',x(a.yr)).attr('x2',x(a.yr)).attr('y1',0).attr('y2',innerH).attr('stroke',C.grid).attr('stroke-dasharray','2 6').attr('opacity',.35);g.append('text').attr('x',x(a.yr)+4).attr('y',18).attr('class','annotation').text(a.txt)});
